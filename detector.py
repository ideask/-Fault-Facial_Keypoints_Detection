...
class get_train_test_set(Dataset):
    ...
    def __init__(self, root_dir, annotations_file, transform=None):
        ...
        file = open(self.annotations_file)
        for f in file:
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.isfile(image_path):
            ...
            return None
        rect_x1 = int(self.names_list[idx].split(' ')[1])
        rect_y1 = int(self.names_list[idx].split(' ')[2])
        rect_x2 = int(self.names_list[idx].split(' ')[3])
        rect_y2 = int(self.names_list[idx].split(' ')[4])
        image = Image.open(image_path)
        image = image.crop((rect_x1, rect_y1, rect_x2, rect_y2))
        img = image.convert("RGB")
        img = img.resize((112, 112))
        img_shape = np.array(img)
        print(img_shape.shape)

        landmarks = list(map(int, list(map(float, self.names_list[idx].split(' ')[5:]))))
...
def main_test():
    ...
    train_set = get_train_test_set(root_dir='/home/kenny/Desktop/Facial_Keypoints_Detection/', annotations_file='/home/kenny/Desktop/Facial_Keypoints_Detection/train.txt', transform=train_transforms)
    test_set = get_train_test_set(root_dir='/home/kenny/Desktop/Facial_Keypoints_Detection/', annotations_file='/home/kenny/Desktop/Facial_Keypoints_Detection/test.txt', transform=test_transforms)
...
if __name__ == '__main__':
    ...
    # #测试dataloader以及data agumentation
    # train_transforms = transforms.Compose([transforms.ToTensor(),])
    # train_set = get_train_test_set(root_dir='/home/kenny/Desktop/Facial_Keypoints_Detection/', annotations_file='train.txt', transform=train_transforms)
    # print(len(train_set))
    # idx = random.randint(0, len(train_set))
    # train_loader = DataLoader(dataset=train_set, batch_size=32, shuffle=True)
    # sample = train_loader.dataset[idx]
    # print(idx, sample['image'].size, sample['landmarks'])
    # img = sample['image']
    # image = transforms.ToPILImage()(img)
    # draw = ImageDraw.Draw(image)
    # x = sample['landmarks'][0::2]
    # y = sample['landmarks'][1::2]
    # landmarks = list(zip(x, y))
    # draw.point(landmarks, fill=(0, 255, 0))
    # plt.imshow(image)
    # plt.pause(1) # 静止
